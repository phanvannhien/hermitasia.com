...
<script>
		var map;
		var mapData = <?php echo $map_data_obj ?>;
	  	jQuery(document).ready(function($) {

	    	var map = $("#map<?php echo $map_id ?>").maps(mapData).data("wpgmp_maps");

	  	});
</script>

<div class="full-width-img">
	<img src="<?php echo get_template_directory_uri().'/image/he-thong-cua-hang.jpg'?>" alt="">
</div>
...
<div class="container">
	<div id="about-page" class="row">
		<div class="content-inner">
			...
			<h3 class="text-white">HỆ THỐNG CỬA HÀNG</h3>
			<form class="form-inline form" method="post" action="" name="">
			<?php
				$map_obj1 = $modelFactory->create_object( 'map' );
				$mapsOptions = $map_obj1->fetch();
				if( count( mapsOptions ) >0 ){
					echo '<select name="map_id" class="form-control">';
					foreach ( $mapsOptions as $map) {
						$selected = ( $map_id == $map->map_id ) ? 'selected' : '';
						echo '<option '.$selected.' value="'.$map->map_id.'">'.$map->map_title.'</option>';
					}
					echo '</select>';
				}
			?>
			...
			<?php
				...
				if ( is_array( $map_locations ) ) {
					...
					foreach ( $map_locations as $location ) {
						if ( !empty( $location->location_group_map ) ) {
							foreach ( $location->location_group_map as $key => $loc_category_id ) {
								if( !in_array($all_categories[ $loc_category_id ], $lstCategories) )
									$lstCategories[] = $all_categories[ $loc_category_id ];
								
							}
						}
					}
				}
				foreach ($lstCategories  as $loc_category) {
					echo '<h3>'.$loc_category->group_map_title.'</h3>';
					if ( is_array( $map_locations ) ) {
						foreach ( $map_locations as $location ) {
							if ( !empty( $location->location_group_map ) ) {
								foreach ( $location->location_group_map as $key => $loc_category_id ) {
									if($loc_category == $all_categories[ $loc_category_id ]){
										echo '<p>'.$location->location_address.'<span class="pull-right">'.$location->location_postal_code.'</span> </p>';
									}
			...
			<div class="col-xl-6 col-lg-6 col-md-12 col-xs-12">
				<div style="height: 500px" id="map<?php echo $map_id ?>"></div>
			</div>
			<p>&nbsp;</p>
		</div>
	</div>
